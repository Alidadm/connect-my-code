...
export const TicTacToeBoard = ({ board, onCellClick, disabled, winningCells = [] }: TicTacToeBoardProps) => {
  return (
    <div className="relative p-4 sm:p-6 rounded-2xl mx-auto w-full max-w-[300px] sm:max-w-[340px]">
      ...
      {/* Decorative corners */}
      <div className="absolute top-2 left-2 w-3 h-3 sm:w-4 sm:h-4 border-t-2 border-l-2 border-primary/50 rounded-tl-lg" />
      <div className="absolute top-2 right-2 w-3 h-3 sm:w-4 sm:h-4 border-t-2 border-r-2 border-primary/50 rounded-tr-lg" />
      <div className="absolute bottom-2 left-2 w-3 h-3 sm:w-4 sm:h-4 border-b-2 border-l-2 border-primary/50 rounded-bl-lg" />
      <div className="absolute bottom-2 right-2 w-3 h-3 sm:w-4 sm:h-4 border-b-2 border-r-2 border-primary/50 rounded-br-lg" />
      
      {/* Game grid */}
      <div className="relative grid grid-cols-3 gap-2 sm:gap-3 w-full aspect-square">
        {board.map((cell, index) => (
          <button
            ...
            className={cn(
              "aspect-square rounded-lg sm:rounded-xl border-2 text-2xl sm:text-3xl md:text-4xl font-bold transition-all duration-300",
