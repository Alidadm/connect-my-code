...
const logStep = (step: string, details?: unknown) => {
  ...
  console.log(`[VERIFY-EMAIL-CHANGE-CODE] ${step}${detailsStr}`);
};
...
serve(async (req) => {
  ...
  try {
    ...
    const newEmail = storedCode.email;
    ...
    // Update profiles_private with new email
    ...
    if (existingPrivate) {
      ...
      await supabaseAdmin
        .from("profiles_private")
        .insert({ user_id: user.id, email: newEmail });
    }

    // Update auth.users email (admin only operation)
    ...
    if (authUpdateError) {
      logStep("Warning: Could not update auth email", { error: authUpdateError.message });
    }
    ...
    logStep("Email updated & verified");
