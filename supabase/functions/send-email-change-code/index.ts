...
const logStep = (step: string, details?: unknown) => {
  ...
  console.log(`[SEND-EMAIL-CHANGE-CODE] ${step}${detailsStr}`);
};
...
serve(async (req) => {
  ...
  try {
    ...
    if (!newEmail) throw new Error("newEmail is required");
    ...
    await resend.emails.send({
      ...
      to: [newEmail],
      subject: "Verify Your New Email Address - DolphySN",
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; margin: 0; padding: 0; background-color: #f4f4f5;">
          <div style="max-width: 500px; margin: 0 auto; padding: 40px 20px;">
            <div style="background: linear-gradient(135deg, #1c76e6 0%, #6366f1 100%); padding: 30px; border-radius: 16px 16px 0 0; text-align: center;">
              ...
              <p style="color: rgba(255,255,255,0.9); margin: 10px 0 0;">Verify Your New Email Address</p>
            </div>
            <div style="background: white; padding: 40px 30px; border-radius: 0 0 16px 16px; box-shadow: 0 4px 20px rgba(0,0,0,0.1);">
              <p style="color: #374151; font-size: 16px; line-height: 1.6; margin: 0 0 20px;">
                You requested to change your email address to <strong>${newEmail}</strong>. Please use the verification code below to confirm:
              </p>
      ...
      `,
      text: `Your DolphySN email change verification code is: ${code}\n\nNew email: ${newEmail}\n\nThis code expires in 10 minutes.\n\nIf you didn't request this change, please ignore this email.`,
    });

    logStep("Email sent to new address");

    return new Response(JSON.stringify({ success: true, message: "Verification code sent to your new email" }), {
